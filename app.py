...
async def callback():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.AnswerFailed"
    ):
        print(request.json[0])
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
    ):
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        print(request.json[0])
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeFailed"
    ):
        print(request.json[0])
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.PlayFailed"
    ):
        print(request.json[0])
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.CallTransferFailed"
    ):
        print(request.json[0])
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.CallConnected"
    ):
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.PlayCompleted"
        and request.json[0].get("data").get("operationContext") == "hang_up"
    ):
        call_automation_client.get_call_connection(call_connection_id).hang_up(
...
async def get_firstname():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "get_firstname"
    ):
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        ...
        if user_response == "":
            ...
            if firstname_error > 2:
                hang_up("misunderstand")

            # play_source = TextSource(
            #     text="Je n'ai pas compris, pouvez-vous répéter votre prénom ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech("fixed_file_source", "repeat_firstname")
            ...
            if (
                ...
                # play_source = TextSource(
                #     text=f"{clean_firstname}, c'est bien ça ?",
                #     voice_name="fr-FR-VivienneMultilingualNeural",
                # )
                play_source = text_to_speech(
                    "file_source", f"{clean_firstname}, c'est bien ça ?"
                )
...
async def get_lastname():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "get_lastname"
    ):
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        ...
        if clean_name is None:
            if lastname_error > 2:
                # play_source = TextSource(
                #     text="Malheureusement, il semblerait que nous n'arrivons pas à nous comprendre. Je vais vous rediriger vers une secrétaire afin de pouvoir accéder a vos requêtes.",
                #     source_locale="fr-FR",
                #     voice_name="fr-FR-VivienneMultilingualNeural",
                # )
                play_source = text_to_speech(
                    "fixed_file_source", "misunderstand_unfortunately"
                )
            ...
            lastname_error += 1
            # play_source = TextSource(
            #     text="Je n'ai pas compris, pouvez-vous épeler votre nom de famille à nouveau ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
...
async def get_birthdate():
    ...
    if (
        ...
        if birthdate is None or is_date_formatted(birthdate) == False:
            # play_source = TextSource(
            #     text="Je n'ai pas compris, quelle est votre date de naissance ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            ...
            # play_source = TextSource(
            #     text=f"Vous confirmez que vous êtes né {date_litterale} ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "file_source", f"Vous confirmez que vous êtes né {date_litterale} ?"
            )
...
async def confirm_firstname():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "confirm_firstname"
    ):
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        ...
        if model_response == "négative":
            firstname_error += 1
            if firstname_error > 2:
                hang_up("misunderstand_unfortunately")

            # play_source = TextSource(
            #     text="Désolé, pouvez-vous me répéter votre prénom ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech("fixed_file_source", "repeat_firstname")
            ...
            # speak(
            #     "Merci, je vous cherche dans notre base patient, laissez moi une petite minute"
            # )
            speak("wait_patient")
            await find_patient()
        else:
            # play_source = TextSource(
            #     text=f"Je n'ai pas compris, {firstname}, c'est bien ça ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "file_source", f"Je n'ai pas compris, {firstname}, c'est bien ça ?"
            )
...
async def confirm_lastname():
    ...
    if (
        ...
        if model_response == "négative":
            ...
            # play_source = TextSource(
            #     text="Désolé, pouvez-vous m'épeler votre nom de famille ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            ...
            # play_source = TextSource(
            #     text=f"Je n'ai pas compris, {lastname}, c'est bien ça ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "file_source", f"Je n'ai pas compris, {lastname}, c'est bien ça ?"
            )
...
async def confirm_birthdate():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "confirm_birthdate"
    ):
        # speak("Très bien.")
        speak("ok")
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        ...
        if model_response == "négative":
            ...
            # play_source = TextSource(
            #     text="Désolé, pouvez-vous me répéter votre date de naissance ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            ...
            count = countPatientInDB(
                {"dateNaissance": {"$regex": f"^{birthdate + 'T00:00:00'}$"}}
            )

            if count > 1 or count == 0:
                # play_source = TextSource(
                #     text="Pouvez-vous m'épeler votre nom de famille ?",
                #     voice_name="fr-FR-VivienneMultilingualNeural",
                # )
        ...
        # play_source = TextSource(
        #     text=f"Je n'ai pas entendu, Vous confirmez que vous êtes né {date_litterale} ?",
        #     voice_name="fr-FR-VivienneMultilingualNeural",
        # )
        play_source = text_to_speech(
            "file_source",
...
async def confirm_call_intent():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "confirm_call_intent"
    ):
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        # speak("D'accord")
        speak("ok2")
        ...
        if model_response == "négative":
            # play_source = TextSource(
            #     text="Il semblerait que je n'ai pas compris votre demande, souhaitez-vous prendre un rendez-vous, modifier un rendez-vous, consulter un rendez-vous planifié, annuler un rendez-vous ou obtenir une information ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            ...
            if (
                ...
                or rdv_intent == "prise de rendez-vous."
            ):
                handle_prise_rdv()
            elif (
                rdv_intent == "modification de rendez-vous"
                or rdv_intent == "modification de rendez-vous."
            ):
                handle_modification()
                ...
                or rdv_intent == "consultation de rendez-vous."
            ):
                handle_consultation()
            ...
            intent_error += 1
            if intent_error > 2:
                hang_up("misunderstand_excuse")

            # play_source = TextSource(
            #     text=f"Pardonnez moi, je n'ai pas compris. Est-ce bien pour un ou une {rdv_intent} ?",
            #     source_locale="fr-FR",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "file_source",
                f"Pardonnez moi, je n'ai pas compris. Est-ce bien pour un ou une {rdv_intent} ?",
            )
    ...
    if (
        ...
        # play_source = TextSource(
        #     text=f"Pardonnez moi, je n'ai pas entendu. Est-ce bien pour un ou une {rdv_intent} ?",
        #     source_locale="fr-FR",
        #     voice_name="fr-FR-VivienneMultilingualNeural",
        # )
        play_source = text_to_speech(
...
async def module_informatif():
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "confirm_rdv"
    ):
        user_response = request.json[0].get("data").get("speechResult").get("speech")
    ...
    if (
        ...
        if model_response == "négative":
            ...
            if type_exam_error <= 2:
                # play_source = TextSource(
                #     text="Pardonnez moi, pouvez-vous me répéter l'intitulé de l'examen que vous souhaitez passer ? ",
                #     source_locale="fr-FR",
                #     voice_name="fr-FR-VivienneMultilingualNeural",
                # )
            ...
            # play_source = TextSource(
            #     text=text, voice_name="fr-FR-VivienneMultilingualNeural"
            # )
            play_source = text_to_speech("file_source", text)
            ...
            # play_source = TextSource(
            #     text="Je ne vous ai pas compris, pour quel type d'examen voulez-vous prendre rendez-vous ? ",
            #     source_locale="fr-FR",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
        ...
        # play_source = TextSource(
        #     text="Je ne vous ai pas entendu. Pouvez-vous répeter l'intitulé de l'examen ?",
        #     voice_name="fr-FR-VivienneMultilingualNeural",
        # )
...
async def rdv_exam_type():
    ...
    if (
        ...
        if exam_type["type_examen"] == None or exam_type["code_examen"] == None:
            # play_source = TextSource(
            #     text="Désolé, je n'ai pas compris. Pouvez-vous répéter l'intitulé de l'examen pour lequel vous souhaitez prendre rendez-vous ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech("fixed_file_source", "repeat_exam_type3")
            ...
            # play_source = TextSource(
            #     text=f"Vous voulez prendre un rendez-vous pour un ou une {exam_type['code_examen']}, c'est bien ça ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "file_source",
                f"Vous voulez prendre un rendez-vous pour un ou une {exam_type['code_examen']}, c'est bien ça ?",
            )
...
async def get_creneaux_choice():
    ...
    if (
        ...
        # play_source = TextSource(
        #     text="D'accord, patientez pendant que je vous réserve ce créneau.",
        #     voice_name="fr-FR-VivienneMultilingualNeural",
        # )
        play_source = text_to_speech("fixed_file_source", "wait_booking")
        ...
        if creneau_choice is None:
            ...
            # play_source = TextSource(
            #     text=f"Je n'ai pas compris le créneau que vous avez choisi. {text}",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "file_source",
            ...
            if matched_creneau is not None:
                ...
                if (
                    ...
                    # play_source = TextSource(
                    #     text=f"Vous avez choisi le {phrase}. Puis-je avoir votre date de naissance ?",
                    #     voice_name="fr-FR-VivienneMultilingualNeural",
                    # )
                    play_source = text_to_speech(
                        "file_source",
    ...
    if (
        ...
        if intent.lower() == "renseignements" or intent.lower() == "renseignements.":
            ...
            if is_question is True:
                ...
                # play_source = TextSource(
                #     text="Bien sûr, posez-moi votre question",
                #     source_locale="fr-FR",
                #     voice_name="fr-FR-VivienneMultilingualNeural",
                # )
            ...
            intent.lower() == "prise de rendez-vous"
            or intent.lower() == "prise de rendez-vous."
        ):
            rdv_intent = intent.lower()

            # speak(
            #     "Je suis désolé, votre requête n'entre pas dans mon champ de compétences, je vous passe un interlocuteur humain."
            # )
            speak("impossible")
            ...
            intent.lower() == "annulation de rendez-vous"
            or intent.lower() == "annulation de rendez-vous."
        ):
            # speak(
            #     "Je suis désolé, votre requête n'entre pas dans mon champ de compétences, je vous passe un interlocuteur humain."
            # )
            ...
            intent.lower() == "consultation de rendez-vous"
            or intent.lower() == "consultation de rendez-vous."
        ):
            # play_source = TextSource(
            ...
            # play_source = TextSource(
            #     text="Désolé, je n'ai pas compris, qui puis-je faire pour vous ?",
            #     source_locale="fr-FR",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
        ...
        # play_source = TextSource(
        #     text="Désolé, je n'ai pas compris, voulez-vous prendre, modifier ou annuler un rendez-vous ?",
        #     source_locale="fr-FR",
        #     voice_name="fr-FR-VivienneMultilingualNeural",
        # )
        play_source = text_to_speech("fixed_file_source", "misunderstand_appointment")
...
async def has_ordonnance():
    ...
    if (
        request.json
        and request.json[0].get("type") == "Microsoft.Communication.RecognizeCompleted"
        and request.json[0].get("data").get("operationContext") == "has_ordonnance"
    ):
        # speak("Très bien")
        speak("ok")
        user_response = request.json[0].get("data").get("speechResult").get("speech")
        ...
        if model_response == "négative":
            hang_up("hang_up_prescription")
        elif model_response == "positive":
            # play_source = TextSource(
            #     text="Quel examen voulez vous passer ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech("fixed_file_source", "ask_exam_type")
            ...
            # play_source = TextSource(
            #     text="Désolé, je n'ai pas compris, Avez-vous une ordonnance ?",
            #     voice_name="fr-FR-VivienneMultilingualNeural",
            # )
            play_source = text_to_speech(
                "fixed_file_source", "misunderstand_prescription"
            )
...
def continue_conversation(model_response):
    ...
    # play_source = TextSource(
    #     text=model_response, voice_name="fr-FR-VivienneMultilingualNeural"
    # )
...
def handle_prise_rdv():
    ...
    # play_source = TextSource(
    #     text="Avez-vous une ordonnance ?", voice_name="fr-FR-VivienneMultilingualNeural"
    # )
    play_source = text_to_speech("fixed_file_source", "ask_prescription")
...
def handle_modification():
    # play_source = TextSource(
    #     text="Pour vous identifier, pouvez-vous me donner votre date de naissance ?",
    #     voice_name="fr-FR-VivienneMultilingualNeural",
    # )
...
def handle_consultation():
    # play_source = TextSource(
    #     text="Très bien. Pouvez-vous me donner votre date de naissance ?",
    #     voice_name="fr-FR-VivienneMultilingualNeural",
    # )
...
def getRDV(patientId):
    ...
    # speak("Finding")
    speak("Finding")
    results = list(rdvCollection.find({"idPatient": patientId}))
...
async def find_patient():
    ...
    results = patientCollection.find(
        {
            "dateNaissance": {"$regex": f"^{birthdate + 'T00:00:00'}$"},
            "nom": {"$regex": f"^{lastname}$", "$options": "i"},  # Case-insensitive
            "prenom": {
    ...
    if first_result is None:
        # play_source = TextSource(
        #     text="Désolé, je ne peux pas donner de rendez-vous à un patient qui n'est pas déjà connu du cabinet. Vous êtes un nouveau patient : Je vous propose de vous transférer à la secrétaire",
        #     source_locale="fr-FR",
        #     voice_name="fr-FR-VivienneMultilingualNeural",
        # )
        ...
        if first_result:
            if (
                ...
                # speak(
                #     "Je vous ai trouvé. Ne quittez pas le temps que je confirme votre rendez-vous."
                # )
                speak("wait_booking2")
                ...
                if rdv.get("success") is True:
                    ...
                    phrase_creneau = full_date_vers_litteral(
                        chosen_creneau.get("date").split("T")[0]
                        + "T"
                        + chosen_creneau.get("heureDebut")
                        + ":00"
                    )

                    speak(
                        f"Parfait, vous avez donc rendez-vous {phrase_creneau} au nom de {lastname}."
                    )
                    continue_conversation("more")
                else:
                    hang_up("hang_up_appointment_error")
                ...
                if len(planned_rdv) == 0:
                    ...
                    # )
                    speak("no_appointment")
                    # play_source = TextSource(
                    #     text="Puis-je faire autre chose pour vous ?",
                    #     source_locale="fr-FR",
                    #     voice_name="fr-FR-VivienneMultilingualNeural",
                    # )
                    play_source = text_to_speech("fixed_file_source", "more")
                    ...
                    speak("has_appointment")
                    dt = datetime.fromisoformat(
                        planned_rdv[0].get("date").split("T")[0]
                        + "T"
                        + planned_rdv[0].get("heure")
                    )
                    ...
                    speak(
                        f"Vous avez rendez-vous le {formatted_date} à {int(hours)} heure {int(minutes)} pour un ou une {result.get('libelle')}."
                    )

                    if (
                        rdv_intent == "modification de rendez-vous"
                        or rdv_intent.lower() == "modification de rendez-vous."
                    ):
                        task_creneaux = asyncio.create_task(
                            get_creneaux_async(
                                sous_type=planned_rdv[0].get("codeExamen"),
                                exam_type=planned_rdv[0].get("typeExamen"),
                            )
                        )
                        # speak(
